...
//	}
	...
	{
		m_Matrix( 1, 0).AddValue(k, minvalue);
	}
	m_Matrix( 1, 0).AddValue(4, -m_TerminiGapOpeningPenalty);
	...
	for( size_t k = 0; k < 8; ++k)
        {
                m_Matrix( 0, 1).AddValue(k, minvalue);
        }
        m_Matrix( 0, 1).AddValue(8, -m_TerminiGapOpeningPenalty);
	...
	for( size_t i = 2; i < m_Matrix.GetNumberOfRows(); ++i)
	{
		...
		m_Matrix( i, 0).AddValue(4, m_Matrix( i-1, 0).GetValue(4) - m_TerminiGapExtensionPenalty);
	...
	for( size_t j = 2; j < m_Matrix.GetNumberOfColumns(); ++j)
	{
		...
		m_Matrix( 0, j).AddValue(8, m_Matrix( 0, j-1).GetValue(8) - m_TerminiGapExtensionPenalty);
	}
	...
	for( size_t i = 1; i < m_Matrix.GetNumberOfRows() -1; ++i)  // first sequence
		for( size_t j = 1; j < m_Matrix.GetNumberOfColumns() -1; ++j)  // second sequence
		{
			...
			if(i<10 && j<10) std::cout << i << "  " << j << "  " << m_Matrix( i, j).GetValue(0) << std::endl;
		}
...
NeedlemanWunschAffineGaps:: TraceBack(int row_end, int col_end) const
{
	...
	if (row_end == -1)
	{
		row_end = m_Matrix.GetNumberOfRows();
	}
	if (col_end == -1)
        {
                col_end = m_Matrix.GetNumberOfColumns();
        }
	if (row_end == 0 or col_end == 0)
	{
		std::vector< std::pair< int, int> > null;
		return std::make_pair(double( 0.0), null);
	}

/*	std::ofstream writescore;
	writescore.open("writescore.txt");

	std::vector< std::vector < double> > VM;
	VM.resize(m_Matrix.GetNumberOfRows());
	for (size_t i = 0; i< m_Matrix.GetNumberOfRows(); i++)
	{
		VM[i].resize(m_Matrix.GetNumberOfColumns());
	...
	for (size_t i = 2; i< m_Matrix.GetNumberOfRows(); i++)
	{
		VM[i-2][0] = m_Matrix(i, 0).GetAffinePathWay(4);
	}
	for (size_t j = 2; j< m_Matrix.GetNumberOfColumns(); j++)
	{
		VM[0][j-2] = m_Matrix(0, j).GetAffinePathWay(8);
	}
	for (size_t i = 2; i< m_Matrix.GetNumberOfRows(); i++)
	{
		for (size_t j = 1; j< m_Matrix.GetNumberOfColumns(); j++)
		{
			if(VM[i-2][j-2] < m_Matrix(i, j).GetAffinePathWay(0)) VM[i-2][j-2] = m_Matrix(i, j).GetAffinePathWay(0);
			if(VM[i-2][j-1] < m_Matrix(i, j).GetAffinePathWay(1)) VM[i-2][j-1] = m_Matrix(i, j).GetAffinePathWay(1);
			if(VM[i-1][j-2] < m_Matrix(i, j).GetAffinePathWay(2)) VM[i-1][j-2] = m_Matrix(i, j).GetAffinePathWay(2);
			if(VM[i-2][j-1] < m_Matrix(i, j).GetAffinePathWay(3)) VM[i-2][j-1] = m_Matrix(i, j).GetAffinePathWay(3);
			if(VM[i-2][j] < m_Matrix(i, j).GetAffinePathWay(4)) VM[i-2][j] = m_Matrix(i, j).GetAffinePathWay(4);
			if(VM[i-1][j-1] < m_Matrix(i, j).GetAffinePathWay(5)) VM[i-1][j-1] = m_Matrix(i, j).GetAffinePathWay(5);
			if(VM[i-1][j-2] < m_Matrix(i, j).GetAffinePathWay(6)) VM[i-1][j-2] = m_Matrix(i, j).GetAffinePathWay(6);
			if(VM[i-1][j-1] < m_Matrix(i, j).GetAffinePathWay(7)) VM[i-1][j-1] = m_Matrix(i, j).GetAffinePathWay(7);
			if(VM[i][j-2] < m_Matrix(i, j).GetAffinePathWay(8)) VM[i][j-2] = m_Matrix(i, j).GetAffinePathWay(8);
	...
	for (size_t i = 0; i< m_Matrix.GetNumberOfRows(); i++)
	{
		for (size_t j = 0; j< m_Matrix.GetNumberOfColumns(); j++)
		{
			writescore << i << "    " << j << "    " << m_Matrix(i, j).GetAffinePathWay(0) << std::endl;
		}
	}
	writescore.close();
*/
	{
		DynamicProgrammingMatrixElement
			element = m_Matrix(row_end - 1, col_end - 1);
