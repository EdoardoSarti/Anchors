...
//	  write_pssm = false;
  ...
  for(std::vector< std::pair< int, int> >::const_iterator t_itr = alignment.begin(); t_itr != alignment.end(); ++t_itr)
  {
	if(t_itr->first < std::numeric_limits< int>::max()-1)
	{
  		number_profiles = FIRST[t_itr->first].GetProfiles().size();
		break;
	}
  }
...
//		std::cout << SECOND[ itr->second].GetType() << "  ";
...
//			std::cout << *vitr << "   ";
		  ...
		  if ( strcmp( &SECOND[ itr->second].GetType(), "-") == 0)
                  {
			for( size_t i = 0; i < number_profiles; ++i)
			{
                        	write.width( 14);
...
//			std::cout << GAP_VALUE << "   ";
	...
	{
		for( size_t i = 0; i < number_profiles*2; ++i)
		{
			write.width( 14);
			write << GAP_VALUE << "  ";
		}
...
//EDO
	  if( VARS.outputfile_aligned_profiles != "" && (! CMD.IsFlagSet( "extract_from_MSA_sequences_with_ids")))
//fine
	  {
	  ...
	  if( CMD.IsFlagSet( "extract_from_MSA_sequences_with_ids"))
	  {
		  if (VARS.first_msa_file != "" && VARS.second_msa_file != "")
		  {
			...
			if (CMD.IsFlagSet( "extract_from_MSA_sequences_with_ids"))
			{
				// Need to build first and second sequence here: start from first and second msa, choose correct id, subtract ignored positions.
				WriteAlignedProfiles( aligned_pair, VARS.first_msa[ ids[ 0]], VARS.second_msa[ ids[ 1]],  VARS.outputfile_aligned_profiles , VARS.gap_extension_penalty, VARS.gap_value, VARS.last_element, VARS.scores, VARS.write_profiles_header1, VARS.write_profiles_header2 );	
			}
		//	  std::ofstream write;
