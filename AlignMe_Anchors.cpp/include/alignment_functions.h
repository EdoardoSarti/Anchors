...
/*
        ...
        for(size_t i = 0; i <= VARS.first_sequence.size(); i++)
        {
                for(size_t j = 0; j <= VARS.second_sequence.size(); j++)
                {
                        writescore << i << "    " << j << "    " << needleman_wunsch_affine_gaps.TraceBack(i, j).first << std::endl;
                }
        }
        writescore.close();
*/
